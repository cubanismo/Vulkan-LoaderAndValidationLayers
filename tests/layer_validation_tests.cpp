...
TEST_F(VkLayerTest, InvalidPushConstants) {
    ...
    const std::array<DuplicateStageFlagsTestCase, 2> duplicate_stageFlags_tests = {
        {
    ...
    for (const auto &iter : duplicate_stageFlags_tests) {
        ...
        m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT, VALIDATION_ERROR_00871);
        err = vkCreatePipelineLayout(m_device->device(), &pipeline_layout_ci, NULL, &pipeline_layout);
        m_errorMonitor->VerifyFound();
        if (VK_SUCCESS == err) {
            vkDestroyPipelineLayout(m_device->device(), pipeline_layout, NULL);
