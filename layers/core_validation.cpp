...
bool ValidateCmdQueueFlags(layer_data *dev_data, GLOBAL_CB_NODE *cb_node, const char *caller_name, VkQueueFlags flags,
    ...
    if (pool) {
        ...
        if (!(flags & queue_flags)) {
            return log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)0, 0, __LINE__,
                           error_code, "DS",
                           "Cannot call %s on a command buffer allocated from a pool without compatible capabilities. %s.",
                           caller_name, validation_error_map[error_code]);
        }
    }
    return false;
}
...
VKAPI_ATTR void VKAPI_CALL CmdPipelineBarrier(VkCommandBuffer commandBuffer, VkPipelineStageFlags srcStageMask,
    ...
    if (cb_state) {
        ...
        skip |= ValidateCmdQueueFlags(dev_data, cb_state, "vkCmdPipelineBarrier()",
